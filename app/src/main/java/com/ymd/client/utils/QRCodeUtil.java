...
public class QRCodeUtil {
     ...
     */
    public static boolean createQRImage(String content, int widthPix, int heightPix, Bitmap logoBm, String filePath) throws WriterException {
        try {
            ...
            int w1 = widthPix - x1 * 2;
            int h1 = heightPix - y1 * 2;

            Bitmap bitmapQR = Bitmap.createBitmap(bitmap, x1, y1, w1, h1);
            //必须使用compress方法将bitmap保存到文件中再进行读取。直接返回的bitmap是没有任何压缩的，内存消耗巨大！
            return bitmapQR != null && bitmapQR.compress(Bitmap.CompressFormat.JPEG, 100, new FileOutputStream(filePath));
        } catch (IOException e) {
            e.printStackTrace();
        }
