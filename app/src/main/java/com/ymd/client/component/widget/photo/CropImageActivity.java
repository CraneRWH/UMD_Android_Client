...
import com.ymd.client.component.widget.photo.cropimage.CropImage;
import com.ymd.client.component.widget.photo.cropimage.CropImageView;
import com.ymd.client.utils.AbConstant;
import com.ymd.client.utils.AbFileUtil;
...
public class CropImageActivity extends BaseActivity implements OnClickListener {
    ...
    private void init() {
        ...
        try {
            if (D) Log.i(TAG, "相片路径是 = " + mPath);
            mBitmap = AbFileUtil.getBitmapFromSD(mFile, AbConstant.SCALEIMG, 500, 500);
            if (mBitmap == null) {
                new SingleToast(CropImageActivity.this).showButtomToast("没有找到图片");
                finish();
            } else {
